...
def build_objective_registry() -> dict[str, ObjectiveTermSpec]:
    return {
        "assignment_cost": ObjectiveTermSpec(
            ...
            description=(
                "Sum of assignment costs for all selected job-machine-slot assignments."
            ),
            evaluator=assignment_cost,
            modeling_hint=(
                "Add the cost coefficient for each selected assignment variable "
                "directly to the MILP objective."
            ),
            suggested_variables=[],
            depends_on=[
                "x[job, machine, slot]",
                "cost[job|machine|slot]",
            ],
            applies_to_objectives=[
                "min_total_cost",
                "min_total_cost_with_changeover",
            ],
        ),
        "changeover_penalty": ObjectiveTermSpec(
            ...
            modeling_hint=(
                ...
                "Add linking constraints z <= x_prev, z <= x_next, and "
                "z >= x_prev + x_next - 1. Then add "
                "changeover_penalty * sum(z) to the MILP objective. "
                "Use eligibility-safe sparse indexing and never reference nonexistent x keys."
            ),
            suggested_variables=[
                "z[machine, slot, prev_job, next_job]",
            ],
            depends_on=[
                "x[job, machine, slot]",
